...
uint32 io_info[PWM_CHANNELS][3] = {
  // MUX, FUNC, PIN
  {PERIPHS_IO_MUX_MTDI_U,  FUNC_GPIO12, 12}, //R
  {PERIPHS_IO_MUX_MTCK_U,  FUNC_GPIO13, 13}, //G
  {PERIPHS_IO_MUX_MTMS_U,  FUNC_GPIO14, 14}, //B
  {PERIPHS_IO_MUX_GPIO5_U, FUNC_GPIO5 ,  5}, //CW
};
  ...
  for (uint8_t color = 0; color < PWM_CHANNELS; color++) {
    if (light_state) {
      if (rgbw[color] != current_rgbw[color] ) {
        ...
        pwm_set_duty((int)(current_rgbw[color] * 16), color);
        pwm_start();
      }
    } else {
...
void setup() {
  ...
  ArduinoOTA.setHostname("LAMP_Schlafzimmer");
